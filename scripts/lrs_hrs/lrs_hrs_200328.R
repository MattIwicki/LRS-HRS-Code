...
raster = rast("data/Poznan/ready/orto_200328.tif")
...
ggplot(smst, aes(x = seg_num, y = lv)) + geom_line() + geom_vline(xintercept = 45, col = "red") + xlab("Number of clusters") + ylab("LV")

# finding peaks

smst = smst[1:148,]

spikes = get_peaks(smst, 45)
length(spikes)
...
ggplot(mean_inh, aes(x = index, y = mean_inh)) + geom_line() + geom_point() + 
  geom_vline(xintercept = 12, col = "gray") + 
  geom_vline(xintercept = 51, col = "gray") + 
...
clustered = cluster_superpixels(slic[, c("L", "A", "B")], 51, w)
plotRGB(raster)
...
# -42.1302517   0.5895791  36.2749061   0.2336350   0.8290243  

# 54
#        AFI    D_index   OMerging   UMerging     recall 
# -5.4352986  0.5438369  3.0673752  0.5929927  0.6794958 
