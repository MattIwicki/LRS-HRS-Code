...
raster = rast("data/Poznan/ready/orto_211001.tif")
...
ggplot(smst, aes(x = seg_num, y = lv)) + geom_line() + geom_vline(xintercept = 55, col = "red") + xlab("Number of clusters") + ylab("LV")
# finding peaks

smst = smst[1:148,]

spikes = get_peaks(smst, 55)
length(spikes)
...
ggplot(mean_inh, aes(x = index, y = mean_inh)) + geom_line() + geom_point() + 
  geom_vline(xintercept = 13, col = "gray") +
  geom_vline(xintercept = 64, col = "gray") + 
...
clustered = cluster_superpixels(slic[, c("L", "A", "B")], 64, w)
plotRGB(raster)
...
# -72.1367279   0.5956582  60.8618881   0.2290670   0.7279815 

# 87
#        AFI    D_index   OMerging   UMerging     recall 
# -1.9730861  0.5556370  0.9903254  0.6501312  0.4260346 
