...
raster = rast("data/Poznan/ready/orto_200406.tif")
...
ggplot(smst, aes(x = seg_num, y = lv)) + geom_line() + geom_vline(xintercept = 49, col = "red") + xlab("Number of clusters") + ylab("LV")

ggplot(smst, aes(x = seg_num, y = roclv)) + geom_line() + geom_vline(xintercept = 49, col = "red") + geom_vline(xintercept = 67, col = "grey") +
  xlab("Number of clusters") + ylab("ROC-LV")

clustered = cluster_superpixels(slic[, c("r", "g", "b")], 67, w)
plot(raster)
