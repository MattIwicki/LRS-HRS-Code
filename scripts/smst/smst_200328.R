...
ggplot(smst, aes(x = seg_num, y = lv)) + geom_line() + geom_vline(xintercept = 38, col = "red") + xlab("Number of clusters") + ylab("LV")

ggplot(smst, aes(x = seg_num, y = roclv)) + geom_line() + geom_vline(xintercept = 38, col = "red") + geom_vline(xintercept = 42, col = "grey") +
  xlab("Number of clusters") + ylab("ROC-LV")

clustered = cluster_superpixels(slic[, c("r", "g", "b")], 42, w)
plot(raster)
